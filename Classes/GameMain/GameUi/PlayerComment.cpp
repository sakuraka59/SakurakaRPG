...
PlayerComment::PlayerComment(std::string comment)
{
	...
	int comment_height = 16 + (this->_FONT_SIZE * this->_comment_line);
	...
	this->_base_sprite->setAnchorPoint(*anchor_point);
	...
	*/
	...
	cocos2d::TTFConfig conf_test("fonts/uzura.ttf", this->_FONT_SIZE);
	cocos2d::Label* label_obj = cocos2d::Label::createWithTTF(conf_test, comment);
	label_obj->setColor(cocos2d::Color3B::WHITE);
	label_obj->setPosition(-20, -70);
	this->addChild(label_obj);
	...
	this->setPosition(this->_BASE_POSITION_X - this->_correct_x + this->_insert_move_x, this->_BASE_POSITION_Y);
...
void PlayerComment::Update(int now_comment_correct_y) {
	...
	// 描画位置の修正
	this->setPosition(this->_BASE_POSITION_X - this->_correct_x + this->_insert_move_x, this->_BASE_POSITION_Y + now_comment_correct_y);
