...
void MapGroundList::Update() {
	...
	if (block_draw_x != this._before_block_draw_x ||
		...
		int draw_block_width = (int)System.Math.Ceiling((double)GameSetting._GAME_WIDTH / (double)GameSetting._MAP_BLOCK_WIDTH);
		int draw_block_height = (int)System.Math.Ceiling((double)GameSetting._GAME_HEIGHT / (double)GameSetting._MAP_BLOCK_HEIGHT);

		int draw_num_x = System.Math.Abs(block_draw_x - this._before_block_draw_x);
		int draw_num_y = System.Math.Abs(block_draw_y - this._before_block_draw_y);
		...
		case 0:
			break;
		case 1:
			map_max_x = DungeonSetting.getDungeonWidth();
			map_max_y = DungeonSetting.getDungeonHeight();
			break;
		}
		for (int check_x = 0; check_x < draw_num_x; check_x++) {
			if (block_draw_x > this._before_block_draw_x) {
				// 描画削除するX座標を取得
				int remove_block_draw_x = this._before_block_draw_x - (int)System.Math.Floor((double)draw_block_width / 2) + check_x + this._map_outer_size_x;
				// 描画追加するX座標を取得
				int add_block_draw_x = this._before_block_draw_x + (int)System.Math.Floor((double)draw_block_width / 2) + 1 + check_x - this._map_outer_size_x;
		...
		for (int check_y = 0; check_y < draw_num_y; check_y++) {
			if (block_draw_y > this._before_block_draw_y) {
				// 描画削除するY座標を取得
				int remove_block_draw_y = this._before_block_draw_y - (int)System.Math.Floor((double)draw_block_height / 2) + check_y + this._map_outer_size_y;
				// 描画追加するY座標を取得
				int add_block_draw_y = this._before_block_draw_y + (int)System.Math.Floor((double)draw_block_height / 2) + 1 + check_y - this._map_outer_size_y;
...
void MapGroundList::reviewGroundBlockX(int add_block_draw_x, int remove_block_draw_x, int block_draw_y, int map_max_y) {
	/*
	int map_draw_size_y = 4 - this._map_outer_size_y;
...
void MapGroundList::setCharaPoint() {
	/*
	int map_width = this._map_data.GetLength(0);
			int map_height = this._map_data.GetLength(1);
			...
			int chara_room_x = 0;
			int chara_room_y = 0;
			int chara_room_num = rand_obj.Next(room_block_count);
